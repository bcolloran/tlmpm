...
* 206-210FPS with leaf block size of 16
* 212-218FPS with leaf block size of 32
* 211-216FPS with leaf block size of 64
* 206-211FPS with leaf block size of 128
Even in the best case, this seems to be a minor regression from skipping the heirarchical layout -- v2.4_just_clean_ups.py gives ~214-219FPS
...
x_world = ti.Vector.field(2, dtype=float)  # position
# is the particle at this position active?
x_active = ti.field(dtype=float)
v = ti.Vector.field(2, dtype=float)  # velocity
F = ti.Matrix.field(2, 2, dtype=float)  # deformation gradient
Pk = ti.Matrix.field(2, 2, dtype=float)  # pk stresses
...
place_particle_field(x_config)
place_particle_field(x_world)
place_particle_field(x_active)
place_particle_field(v)
